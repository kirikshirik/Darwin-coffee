...
Прогоняет весь конвейер отчётов так же, как это сделает бот:
  1) сверяет, что помесячный честный P&L совпадает с контролем honest_report (334 651 ₽);
...
# 334 651 ₽: после гарда неполного майского food cost (actuals_data.effective_food_cost) —
# май берёт реалистичную оценку COGS вместо недовнесённых 25 000 ₽.
HONEST_ANNUAL_CONTROL = Decimal("334651")
