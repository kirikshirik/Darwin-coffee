...
3. На шаге переменных заполни те, что помечены `sync:false`:
   ...
   | `TELEGRAM_OWNER_CHAT_ID` | chat_id владельца (@userinfobot). Несколько — через запятую: `111,222` |
...
> Даже без пингера данные не «протухнут»: при открытии `/dashboard` бот фоном подтягивает
> свежие чеки Эвотора (`_maybe_bg_sync` в [backend/bot/health.py](../backend/bot/health.py)),
> если давно не синкали. UptimeRobot нужен, чтобы **плановый** синк (каждые 15 мин) и утренняя
> сводка работали и тогда, когда дашборд никто не открывает.
